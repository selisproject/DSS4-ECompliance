...
using DSS4_ECompliance.UI.ViewModels.PermissionsList;
...
namespace DSS4_ECompliance.UI.Controllers.PermissionsList
{
...
// Predefined Ordering
        ...
        {
            __request = __request ?? new DatasourceRequest(0, int.MaxValue);
            var __items = new DSS4_ECompliance.DAL.Repository().GetAsQueryable<zAppDev.DotNet.Framework.Identity.Model.ApplicationPermission>().AsQueryable();
...
//Fix for total items in GetGroupsClosed
        ...
        public JsonResult List_Export()
        {
            ...
            var aggregatorsInfo = DeserializeAggregatorsRequest<zAppDev.DotNet.Framework.Identity.Model.ApplicationPermission>(exportOptions.AggregatorsRequest);
        ...
        [NonAction]
        public string List_ExportImplementation(DatasourceRequest datasourceRequest, ExportHelper.ExportOptions exportOptions, List<AggregatorInfo<zAppDev.DotNet.Framework.Identity.Model.ApplicationPermission>> aggregatorsInfo)
        {
            ...
            var items = DatasourceRetriever.Retrieve(datasourceRequest, queryable);;
            var dto = items.Select(i => new List1DataSet_ApplicationPermissionDTO(i));
            var formattings = new Dictionary<string, string>();
            var aggregators = RuntimePredicateBuilder.BuildAggregatorPredicates(aggregatorsInfo);
            foreach (var a in aggregators)
            {
            ...
            foreach (var record in dto)
            {
                ...
                recordDTO.Columns.Add(new ExportHelper.ExportColumnDTO
                {
                    ...
                    Format = "",
                    ExcelFormat = @"",
                    Caption = BaseViewPage<object>.GetResourceValue("PermissionsList", "RES_LIST_List_COLUMN_Name")
                });
                recordDTO.Columns.Add(new ExportHelper.ExportColumnDTO
                {
                    ...
                    Format = "",
                    ExcelFormat = @"",
                    Caption = BaseViewPage<object>.GetResourceValue("PermissionsList", "RES_LIST_List_COLUMN_Description")
                });
                recordDTO.Columns.Add(new ExportHelper.ExportColumnDTO
                {
                    ...
                    Format = "",
                    ExcelFormat = @"",
                    Caption = BaseViewPage<object>.GetResourceValue("PermissionsList", "RES_LIST_List_COLUMN_IsCustom")
                });
            ...
            return FileHelper.PendingDownloadInstance.Add("PermissionsList", content, fileName);
        }
        ...
        [NonAction]
        public string List_ExportV2Implementation(DatasourceRequest datasourceRequest, ExportOptionsV2 exportOptions, List<AggregatorInfo<zAppDev.DotNet.Framework.Identity.Model.ApplicationPermission>> aggregatorsInfo)
        {
